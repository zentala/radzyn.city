...
export async function GET() {
  ...
  if (cache && Date.now() - cache.timestamp < CACHE_TIME) {
    return NextResponse.json(cache.data, {
      headers: {
        // CDN/proxy/browser cache for 1h, allow stale while revalidating
  ...
  const res = await fetch(url.toString(), {
    // Avoid caching at the edge; we cache in-memory above.
    cache: 'no-store',
  });
  ...
  if (!res.ok) {
    ...
    return NextResponse.json(
      { error: 'Failed to fetch current weather', details: text },
      { status: 502 }
    );
  }
